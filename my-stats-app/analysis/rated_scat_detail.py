...
comparisons = [
    ('Q15: rate exercise','Q16: rate diet',  cm.Reds),
    ('Q15: rate exercise','Q17: rate sleep', cm.Blues),
    ('Q16: rate diet',    'Q17: rate sleep', cm.Greens)
]
...
for key, (csv_path, title) in datasets.items():
    ...
    # shared max frequency for consistent colour scale
    freq_tables, maxfreq = {}, 0
    for xcol, ycol, _ in comparisons:
    ...
    for ax, (xcol, ycol, base_cmap) in zip(axes, comparisons):
        ...
        ax.set_title(f"{short_name[xcol]} vs {short_name[ycol]}", fontsize=12, pad=14)
        ax.text(0.5, 1.02,
                f"r={res.rvalue:.2f}, R²={res.rvalue**2:.2f}  "
                f"a={res.slope:.2f}, b={res.intercept:.2f}",
    ...
    fig.suptitle(f"{title}\nInter-correlations of Health-style Ratings",
                 fontsize=12, fontweight='bold', y=1.00)
    plt.tight_layout()
    os.makedirs(f'plots/{key}', exist_ok=True)
    fig.savefig(f'plots/{key}/{key}_scatter_freq_tri.png',
